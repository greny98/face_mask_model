...
class AnchorBoxes:
    def __init__(self):
        ...
        step = int((120 - 16) / 4)
        self._areas = [(x * step + 16) ** 2 for x in range(4)]
    ...
    def _get_anchors(self, feature_width, feature_height, level):
        ...
        centers = tf.stack(tf.meshgrid(rx, ry), axis=-1) * self._strides[level - 4]
        centers = tf.expand_dims(centers, axis=-2)
        centers = tf.tile(centers, [1, 1, self._num_anchors, 1])
        dims = tf.tile(self._anchor_dims[level - 4], [feature_width, feature_height, 1, 1])
